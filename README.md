...
- **任务栏状态条**：显示在任务栏最左侧（自动避让播放器歌词等贴任务栏控件）
  ...
  - 左键弹出详情面板，右键弹出菜单
- **托盘图标**：彩色状态点 + 思考中旋转动画
  - 🔵 思考中（模型生成）· 🟠 工具执行（含当前工具名与耗时）· 🟢 等待输入（挂着提问/计划审批等用户操作的会话）· ✔️ 灰绿 已完成 · 🔴 出错 · ⚪ 空闲
...
- 「等待输入」只显示**挂着提问/审批**（轮次未结束）的会话：AskUserQuestion/计划审批期间、开放轮次静默 >90s。单纯开着的会话（新建/恢复后没发消息、问完已收尾）一律不展示——ZCode 日志没有"会话关闭"事件，静默无法区分"在等用户"与"已关终端"，只有挂起提问有明确标记（轮次未结束），因此关闭的会话不再滞留，也无需 15 分钟淡出
- 统计口径为"启动回看窗口 + 本次运行增量"，重启后今日计数从种子窗口重新累积
- `turn.started` 若发生在种子窗口之前，轮次数会偏小
- 会话显示最近 12 小时内活跃、且**运行中或挂着提问/审批**的（最多 8 个）；其余在后台跟踪，再次 turn.started 会重新出现；所有会话都空闲时总体显示空闲
- 事件流停滞 >90s 自动降级（轮次未结束判等待输入，已结束判空闲；防止 ZCode 异常退出后状态卡在"运行中"）
